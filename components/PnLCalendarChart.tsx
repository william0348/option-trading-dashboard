    ...
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[14px] font-black text-slate-500 uppercase tracking-widest">Daily P&amp;L Calendar</span>
          {selectedDay && (
            <span className="px-2 py-0.5 bg-indigo-100 text-indigo-600 rounded-full text-[11px] font-black">
              {selectedDay}
            </span>
          )}
        </div>
        <div className="flex items-center gap-1">
          ...
          <span className="text-[13px] font-black text-slate-700 min-w-[120px] text-center">{monthLabel}</span>
          ...
          {selectedDay && (
            <button onClick={() => onDayClick(null)}
              className="ml-2 px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg text-[11px] font-black uppercase tracking-widest transition-colors">
              Clear
            </button>
          )}
        </div>
      ...
      {monthStats.count > 0 && (
        <div className="flex items-center gap-3 mb-3 px-3 py-2 bg-slate-50 rounded-xl">
          <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest">{monthLabel} Summary</span>
          <span className={`text-[13px] font-black ${monthStats.pnl >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
            {formatPnl(monthStats.pnl)}
          </span>
          <span className="text-[11px] font-bold text-slate-400">{monthStats.count} trades</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-7 gap-1 mb-1">
        {DAYS_OF_WEEK.map(d => (
          <div key={d} className="text-center text-[10px] font-black text-slate-300 uppercase tracking-widest py-1">
            {d}
          </div>
        ))}
      </div>
        ...
        {cells.map((cell, i) => {
          ...
          return (
            ...
            >
              ...
              {hasTrades && (
                <>
                  <span className={`text-[9px] font-bold block mt-0.5 leading-tight ${
                    isProfit ? 'text-emerald-600' : 'text-rose-600'
                  }`}>
                    {formatPnl(data.pnl)}
                  </span>
                  <span className="absolute bottom-1 right-1 text-[9px] font-black bg-white/80 rounded px-0.5 text-slate-500">
                    {data.count}t
                  </span>
                </>
              )}
      ...
      <div className="flex items-center gap-4 mt-3 pt-3 border-t border-slate-50">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Profit Day</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-3 rounded bg-rose-100 border border-rose-200"/>
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Loss Day</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-3 rounded bg-slate-100 border border-slate-200"/>
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">No Trade</span>
        </div>
        <span className="ml-auto text-[10px] font-bold text-slate-300">Click day to filter table</span>
      </div>
    </div>
  );
};
